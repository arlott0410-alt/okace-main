/**
 * GET /api/cache/shifts
 * Edge cache สำหรับรายการกะ (active) — ลดการยิง Supabase
...
const KV_KEY = 'cache:shifts';
const TTL_SECONDS = 300; // 5 min
const SELECT = 'id,name,code,start_time,end_time,sort_order,active,created_at,updated_at';
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const res = await fetch(`${cfg.base}/rest/v1/shifts?active=eq.true&select=${SELECT}&order=sort_order.asc`, {
    headers: {
      apikey: cfg.apikey,
      Authorization: auth,
